...
type RewardPointData = {
  totalPoint: number;
  rewardItems: RewardItem[];
  rewardInfo: RewardInfo;
};

type OrderSheetPageProps = {
  pointMoneyData?: PointMoneyData;
  rewardPointData?: RewardPointData;
};

const MOCK_REWARD_POINT_DATA: RewardPointData = {
...
export function OrderSheetPage({
  pointMoneyData = MOCK_POINT_MONEY_DATA,
  rewardPointData = MOCK_REWARD_POINT_DATA,
}: OrderSheetPageProps = {}) {
  return (
    <div className='flex min-h-screen flex-col gap-[31px] bg-gray-300'>
      ...
      <main className='flex flex-col gap-[40px] px-[16px] pb-[16px]'>
        <PointMoneySection {...pointMoneyData} />
        <RewardPointSection {...rewardPointData} />
        <AgreementSection />
